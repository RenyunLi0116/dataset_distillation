...
def run(args):
    ...
    for step in range(max_steps + 1):
        ...
        # v2 update: loss, psnr, lpips
        loss_mse = F.mse_loss(rgb, pixels)
        psnr = -10.0 * torch.log(loss_mse) / np.log(10.0)
        loss_store.append(loss_mse.item())
        psnr_store.append(psnr.item())

        # v3 update
        if step > 0:
            rate = max(abs((loss_mse.item() - prev_loss)/prev_loss), abs((psnr.item() - prev_psnr)/prev_psnr))
            if prev_rate is None:
                prev_rate = rate
            if prev_rate is not None and step > (max_steps // 10) and rate > prev_rate:
                # clear significant_changes
                significant_changes['loss'] = []
                significant_changes['psnr'] = []
                significant_changes['data'] = []
                significant_changes['loss'].append(step)
                significant_changes['psnr'].append(step)
                significant_changes['data'].append(data)
            
            # if abs((loss_mse.item() - prev_loss)/prev_loss) > 0.2 or abs((psnr.item() - prev_psnr)/prev_psnr) > 0.2:
            #     significant_changes['loss'].append(step)
            #     significant_changes['psnr'].append(step)
            #     significant_changes['data'].append(data)
        prev_loss = loss_mse.item()
        prev_psnr = psnr.item()
    ...
    np.savetxt("psnr_store_dramatic_v3.txt", np.array(psnr_store))
    np.savetxt("loss_store_dramatic_v3.txt", np.array(loss_store))
    # np.save('significant_changes.npy', significant_changes)
    np.save('psnr.npy', significant_changes['psnr'])
    np.save('loss.npy', significant_changes['loss'])
    np.save('data.npy', significant_changes['data'])
    ...
    plt.savefig('lpips_store_v3.png')
    plt.close()
