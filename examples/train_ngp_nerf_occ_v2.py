...
def run(args):
    ...
    for step in range(max_steps + 1):
        ...
        # lpips = lpips_fn(rgb, pixels)
        # loss_store.append(loss.item())
        loss_store.append(loss_mse.item())
        psnr_store.append(psnr.item())
        # lpips_store.append(lpips_fn(rgb, pixels).item())


        if step % 1000 == 0:
            print(f"loss len: {len(loss_store)}, psnr len: {len(psnr_store)}, lpips len: {len(lpips_store)}")
    ...
    np.savetxt("psnr_store.txt", np.array(psnr_store))
    np.savetxt("lpips_store.txt", np.array(lpips_store))
    np.savetxt("loss_store.txt", np.array(loss_store))
